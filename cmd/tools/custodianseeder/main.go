...
func seedCustodians(logger *slog.Logger, dbPool *pgxpool.Pool) error {

	inputName := "custodians.csv"
	...
	logger.Info("Start import of custodian data...")
	...
	for {
		...
		shortcode := record[0]
		name := record[1]
		countryShortcode := record[2]

		country, err := baseQueries.GetCountryByShortcode(ctx, countryShortcode)

		if err == nil && len(shortcode) <= 5 {
			...
			logger.Error("Country not found", "country", shortcode, "error", err)
		}
	}
...
func main() {
	...
	if err := seedCustodians(logger, dbPool); err != nil {

		logger.Error("Error initializing custodians", "error", err)
	}
}
