...
func seedExchanges(logger *slog.Logger, dbPool *pgxpool.Pool) error {

	inputName := "exchanges.csv"
	...
	logger.Info("Start import of exchange data...")
	...
	for {
		...
		if len(shortcode) <= 8 {
			...
			_, err = baseQueries.InsertExchange(ctx, rbsdb.InsertExchangeParams{
...
func main() {
	...
	if err := seedExchanges(logger, dbPool); err != nil {

		logger.Error("Error initializing exchanges", "error", err)
	}
}
